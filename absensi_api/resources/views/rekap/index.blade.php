...
<div class="min-h-screen bg-slate-50 font-['Plus_Jakarta_Sans']">
    <!-- Navbar -->
    <nav class="sticky top-0 z-50 bg-white/70 backdrop-blur-md border-b border-slate-200 px-4 py-4 sm:px-6">
        <div class="max-w-7xl mx-auto flex flex-col gap-4 lg:flex-row lg:justify-between lg:items-center">
            <div class="flex items-center space-x-3">
                <a href="{{ route('dashboard') }}" class="w-11 h-11 sm:w-10 sm:h-10 bg-blue-600 rounded-xl flex shrink-0 items-center justify-center shadow-lg shadow-blue-200 text-white">
                    <i class="fas fa-arrow-left"></i>
                </a>
                <div class="min-w-0">
                    <h1 class="text-lg sm:text-xl font-extrabold tracking-tight text-slate-800">REKAP <span class="text-blue-600">ABSENSI</span></h1>
            ...
            <div class="grid grid-cols-1 gap-3 sm:grid-cols-3 lg:flex lg:items-center lg:space-x-4 lg:gap-0">
                <form action="{{ route('rekap.index') }}" method="GET" id="filterForm" class="flex items-center">
                    ...
                    <input type="date" name="date" value="{{ $date }}" onchange="this.form.submit()" class="min-h-11 w-full bg-slate-100 border-none rounded-xl px-4 py-2 text-sm font-bold text-slate-600 focus:ring-2 focus:ring-blue-500 transition-all">
                </form>
                <form action="{{ route('rekap.bulk-hadir') }}" method="POST" onsubmit="return confirm('Tandai semua {{ $selectedRole }} hadir untuk tanggal ini?')">
                    ...
                    <input type="hidden" name="role" value="{{ $selectedRole }}">
                    <button type="submit" class="min-h-11 w-full justify-center bg-blue-100 hover:bg-blue-200 active:bg-blue-200 text-blue-700 px-4 py-2 rounded-xl text-sm font-bold flex items-center transition-all">
                        <i class="fas fa-check-double mr-2"></i> Set Semua Hadir
                    </button>
                </form>
                <a href="{{ route('rekap.export', ['date' => $date, 'role' => $selectedRole]) }}" class="min-h-11 justify-center bg-emerald-500 hover:bg-emerald-600 active:bg-emerald-600 text-white px-4 py-2 rounded-xl text-sm font-bold flex items-center transition-all shadow-lg shadow-emerald-100">
    ...
    <main class="max-w-7xl mx-auto px-4 py-6 sm:px-6 sm:py-10">
        ...
        <div class="grid grid-cols-2 gap-3 mb-6 sm:flex sm:space-x-4 sm:gap-0 sm:mb-8">
            <a href="{{ route('rekap.index', ['date' => $date, 'role' => 'siswa']) }}" 
               class="min-h-12 px-4 sm:px-6 py-3 rounded-2xl font-bold text-sm transition-all flex items-center justify-center text-center {{ $selectedRole == 'siswa' ? 'bg-blue-600 text-white shadow-lg shadow-blue-200' : 'bg-white text-slate-500 hover:bg-slate-50' }}">
                <i class="fas fa-user-graduate mr-2"></i> Siswa
            </a>
            <a href="{{ route('rekap.index', ['date' => $date, 'role' => 'guru']) }}" 
               class="min-h-12 px-4 sm:px-6 py-3 rounded-2xl font-bold text-sm transition-all flex items-center justify-center text-center {{ $selectedRole == 'guru' ? 'bg-blue-600 text-white shadow-lg shadow-blue-200' : 'bg-white text-slate-500 hover:bg-slate-50' }}">
        ...
        <div class="bg-white rounded-2xl sm:rounded-3xl shadow-sm border border-slate-100 overflow-hidden">
            <div class="p-5 sm:p-8 border-b border-slate-50 flex flex-col gap-1 sm:flex-row sm:justify-between sm:items-center">
                <h3 class="text-lg sm:text-xl font-extrabold text-slate-800">Daftar Absensi {{ ucfirst($selectedRole) }}</h3>
                <span class="text-xs sm:text-sm font-bold text-slate-400 uppercase tracking-widest">{{ \Carbon\Carbon::parse($date)->format('d F Y') }}</span>
            </div>
                    ...
                    <div class="p-5">
                        <div class="flex items-start justify-between gap-3">
                            <div class="flex min-w-0 items-center">
                                <div class="w-11 h-11 rounded-full bg-blue-50 text-blue-600 flex shrink-0 items-center justify-center font-bold text-sm mr-3">
                            ...
                            <span class="shrink-0 px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-wider {{ $statusClasses[$item->status] ?? 'bg-slate-50 text-slate-600' }}">
                                {{ $item->status }}
                            </span>
                        </div>
                        ...
                        <div class="mt-5 grid grid-cols-2 gap-3">
                            <div class="rounded-xl bg-slate-50 px-3 py-3">
                                <span class="block text-[10px] font-black uppercase tracking-widest text-slate-400">Masuk</span>
                                <span class="mt-1 block text-sm font-extrabold {{ $item->time_in == '-' ? 'text-slate-300' : 'text-slate-700' }}">{{ $item->time_in }}</span>
                            </div>
                            <div class="rounded-xl bg-slate-50 px-3 py-3">
                                <span class="block text-[10px] font-black uppercase tracking-widest text-slate-400">Pulang</span>
                                <span class="mt-1 block text-sm font-extrabold {{ $item->time_out == '-' ? 'text-slate-300' : 'text-slate-700' }}">{{ $item->time_out }}</span>
                            </div>
                        </div>
                        ...
                        <form action="{{ route('rekap.update') }}" method="POST" class="mt-4">
                            ...
                            <label class="block text-[10px] font-black uppercase tracking-widest text-slate-400 mb-2">Ubah Status</label>
                            <select name="status" onchange="this.form.submit()" class="min-h-11 w-full bg-slate-100 border-none rounded-xl px-4 py-2 text-sm font-bold text-slate-600 focus:ring-2 focus:ring-blue-500 cursor-pointer">
